...
Ustawienia aplikacji (motyw, gęstość interfejsu, przełącznik hipopotama) obsługuje `AppSettingsStoreService` (`src/app/services/app-settings-store/`) — klucz `settings` przechowuje obiekt `AppSettings`. To samo uprawnienie `store:default` obejmuje oba pliki.
