    ...
    <div class="kpi-strip">
      <div class="kpi">
        ...
        <div class="kpi-val mono">
          {{ calc.schedule().firstInstallment | pln }}<span class="kpi-unit">zł</span>
        </div>
        <div class="kpi-meta">
          {{ calc.installmentType() }} · {{ calc.rateType() }} {{ effRate() | number: '1.2-2' }}%
        </div>
      </div>
      <div class="kpi">
        <div class="kpi-lab">Suma wszystkich płatności</div>
        <div class="kpi-val mono">
          {{ calc.schedule().totalPayments | pln }}<span class="kpi-unit">zł</span>
        </div>
        <div class="kpi-meta">
      ...
      <div class="kpi kpi--cost">
        ...
        <div class="kpi-meta">
          prowizja {{ calc.schedule().commission | pln }} · wycena
          {{ calc.schedule().valuationFee | pln }}
        </div>
      </div>
    </div>
  `,
