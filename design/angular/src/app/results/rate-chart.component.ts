...
interface RateChartGeo {
  ...
  bands: { kind: RateBand['kind']; x: number; y: number; width: number; height: number; label: string; color: string; opacity: number; showLabel: boolean; labelX: number; labelY: number }[];
      ...
      <div class="card card--rate">
        <div class="trend-head rate-head">
          ...
          <ul class="trend-legend rate-legend">
            <li>
              <span class="line-glyph">
                <svg width="28" height="10">
                  <path d="M2 7 H10 V3 H18 V7 H26" stroke="var(--ink)" stroke-width="2" fill="none"/>
            ...
            @if (hasKind('period')) {
              <li><span class="dot" style="background:var(--accent); opacity:0.5"></span>okres oprocentowania</li>
            }
            @if (hasKind('bridge')) {
        ...
        <svg [attr.viewBox]="'0 0 ' + g.w + ' ' + g.h" class="rate-chart" preserveAspectRatio="none">
          <!-- gridlines + lewa oś -->
          ...
          <text [attr.x]="g.padL - 70" [attr.y]="g.padT + g.innerH / 2"
                ...
                class="ax-title" text-anchor="middle">Nominalne oprocentowanie roczne</text>
          ...
          @for (b of g.bands; track $index) {
            <g>
              ...
              @if (b.showLabel) {
                <text [attr.x]="b.labelX" [attr.y]="b.labelY"
                      class="rate-band-label" fill="var(--ink-2)">{{ b.label }}</text>
              }
            </g>
          }
          ...
          @for (c of g.changes; track $index) {
            <g>
              <circle [attr.cx]="c.cx" [attr.cy]="c.cy" r="4"
                      fill="var(--ink)" stroke="var(--surface)" stroke-width="1.5"/>
              <line [attr.x1]="c.cx" [attr.x2]="c.cx"
                    [attr.y1]="c.cy - 6" [attr.y2]="c.cy - c.tickEnd"
                    stroke="var(--line-2)"/>
              <text [attr.x]="c.cx" [attr.y]="c.cy - c.labelOffset"
                    class="rate-change-label" text-anchor="middle">{{ c.label }}</text>
            </g>
          }
...
export class RateChartComponent {
  ...
  geo = computed<RateChartGeo | null>(() => {
    ...
    BAND_ORDER.forEach(k => { if (rateBands.some(b => b.kind === k)) usedKinds.push(k); });
    const trackH = 12;
    const trackGap = 3;
    const tracksTop = 4;
    ...
    return { w, h, padL, padR, padT, padB, innerW, innerH, yTicks, xTicks, linePath, areaPath, bands, changes };
  });
}
