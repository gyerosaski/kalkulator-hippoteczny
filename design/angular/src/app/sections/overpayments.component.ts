...
@Component({
  ...
  template: `
    <app-section title="Nadpłaty" [defaultOpen]="true"
      ...
      <div class="cost-sub">
        ...
        <div class="row row--2" id="field-over-dates">
          <app-field label="Data nadpłaty — od" num="2">
            <app-month-picker [value]="calc.overFrom()" (valueChange)="calc.overFrom.set($event)"/>
          </app-field>
          <app-field label="do">
            <app-month-picker [value]="calc.overTo()" (valueChange)="calc.overTo.set($event)"/>
          </app-field>
        </div>
      ...
      <div class="cost-sub">
        ...
        <div class="row row--2">
          <app-field label="Chcę co miesiąc płacić do banku ratę w wysokości" num="5">
            <app-number-input [value]="calc.targetRata()" (valueChange)="calc.targetRata.set($event)"
              suffix="zł" [decimals]="2"/>
          </app-field>
          <app-field label="Skutek nadpłaty" num="7">
            <app-select [options]="['niższa rata','skrócenie okresu']" [value]="calc.targetEffect()"
        ...
        <div class="row row--2" id="field-target-dates">
          <app-field label="Data nadpłaty — od" num="6">
            <app-month-picker [value]="calc.targetFrom()" (valueChange)="calc.targetFrom.set($event)"/>
          </app-field>
          <app-field label="do">
            <app-month-picker [value]="calc.targetTo()" (valueChange)="calc.targetTo.set($event)"/>
          </app-field>
        </div>
      </div>
      ...
      <div class="cost-sub">
        <div class="cost-sub-head"><span class="cost-sub-num">C</span>Prowizja za wcześniejszą spłatę</div>
        <div class="row row--2">
          <app-field label="Wysokość prowizji za wcześniejszą spłatę" num="8">
            <app-number-input [value]="calc.earlyRepayFee()" (valueChange)="calc.earlyRepayFee.set($event)"
              suffix="%" [decimals]="2"/>
          </app-field>
          <app-field label="Bank pobiera prowizję do" num="9">
            <app-month-picker [value]="calc.earlyRepayFeeUntil()" (valueChange)="calc.earlyRepayFeeUntil.set($event)"/>
          </app-field>
        </div>
      </div>
