...
interface KpiMetric {
  key: string; label: string; tone: 'primary' | 'cap' | 'int' | 'cost';
  aVal: number; bVal: number; aMeta: string; bMeta: string;
}
interface DiffRow {
  lab: string; aV: number; bV: number; invert: boolean; unit: string;
  emphasis?: boolean; fmt?: (v: number) => string;
  deltaClass: string; aLeader: boolean; bLeader: boolean; deltaTxt: string;
  aTxt: string; bTxt: string;
}
    ...
    <div class="cmp-view">
      ...
      <header class="cmp-hero">
        <div class="cmp-hero-text">
          <h1 class="cmp-hero-title">Porównanie ofert</h1>
          <div class="cmp-hero-meta">
            <span>A — baza</span>
            <span>B — porównywana</span>
            <span class="mono">Δ = B − A</span>
          </div>
        </div>

        <div class="cmp-slots">
          <ng-container *ngTemplateOutlet="slotTpl; context: { side: 'A', offer: calc.offerA() }"></ng-container>

          <button class="cmp-swap" (click)="calc.swapComparison()" title="Zamień strony (A ↔ B)" aria-label="Zamień strony">
            <svg width="20" height="20" viewBox="0 0 20 20">
              <path d="M4 7 L14 7 M11 4 L14 7 L11 10" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M16 13 L6 13 M9 16 L6 13 L9 10" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </button>

          <ng-container *ngTemplateOutlet="slotTpl; context: { side: 'B', offer: calc.offerB() }"></ng-container>
        </div>
      </header>
      ...
      @if (!calc.offerA() || !calc.offerB()) {
        ...
        <div class="cmp-empty cmp-empty--v2">
          ...
          <ol class="cmp-empty-steps" aria-label="Postęp porównania">
            <li class="cmp-empty-step cmp-empty-step--a"
                [class.is-current]="step === 1"
                [class.is-done]="step > 1">
              <span class="cmp-empty-step-num">
                @if (step > 1) {
                  <svg width="10" height="10" viewBox="0 0 10 10"><path d="M2 5 L4.5 7.5 L8.5 2.5" stroke="currentColor" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>
                } @else { 1 }
              </span>
            ...
            <li class="cmp-empty-step cmp-empty-step--b"
          ...
          <div class="cmp-empty-hero">
            ...
            <h2 class="cmp-empty-hero-title">
              ...
              <span class="cmp-empty-hero-pill" [ngClass]="'cmp-empty-hero-pill--' + lower(targetSlot)"
            ...
            <p class="cmp-empty-hero-sub">
              @if (step === 1) {
                Kliknij jedną z zapisanych kalkulacji poniżej. Stanie się <b>bazą</b> porównania (lewa kolumna).
              } @else {
                Zostało wybrać kalkulację porównywaną (prawa kolumna). Wszystkie różnice (Δ) liczymy jako <span class="mono">B − A</span>.
              }
            </p>
          </div>
          ...
          @if (hasEnough && visible.length > 0) {
            ...
            <div class="cmp-empty-list">
              @for (o of visible; track o.id) {
                <button class="cmp-empty-card" [ngClass]="'cmp-empty-card--' + lower(targetSlot)"
                  (click)="pickInline(targetSlot, o.id)">
                  <span class="cmp-empty-card-pin" [ngClass]="'cmp-empty-card-pin--' + lower(targetSlot)">{{ targetSlot }}</span>
                  <div class="cmp-empty-card-name">{{ o.name }}</div>
                  <div class="cmp-empty-card-grid">
                    <div><div class="muted xs">kwota</div><div class="mono">{{ fmt0(o.source.loanAmount) }} zł</div></div>
                    <div><div class="muted xs">okres</div><div class="mono">{{ periodTxt(o.source.years, o.source.months) }}</div></div>
                    <div><div class="muted xs">oproc.</div><div class="mono">{{ fmtPct(o.source.rate, 2) }}%</div></div>
                    <div><div class="muted xs">rodzaj</div><div class="cmp-empty-card-rt">{{ o.source.rateType }}</div></div>
                  </div>
                  <div class="cmp-empty-card-foot">
                    <div class="cmp-empty-card-rata">
                      <span class="muted xs">pierwsza rata</span>
                      <span class="mono"><b>{{ fmt0(o.result.firstInstallment) }}</b> zł</span>
                    </div>
            ...
            <button class="cmp-empty-modal-btn" (click)="openPicker(targetSlot)">
              <svg width="13" height="13" viewBox="0 0 14 14">
                ...
                <path d="M9.5 9.5 L12.5 12.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
              </svg>
          ...
          @if (!hasEnough) {
            <div class="cmp-empty-nosaved">
              <svg width="60" height="60" viewBox="0 0 60 60" aria-hidden="true">
                <rect x="10" y="14" width="40" height="32" rx="3" fill="none" stroke="currentColor" stroke-width="1.4"/>
                <path d="M16 22 L36 22 M16 28 L30 28 M16 34 L40 34 M16 40 L24 40" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
                <circle cx="48" cy="42" r="6" fill="var(--surface)" stroke="currentColor" stroke-width="1.4"/>
                <path d="M48 39.5 L48 44.5 M45.5 42 L50.5 42" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
              </svg>
              <div class="cmp-empty-title">
                {{ calc.availableOffers().length > 0 ? 'Masz tylko jedną zapisaną kalkulację' : 'Brak zapisanych kalkulacji' }}
              </div>
              <div class="cmp-empty-sub">
                Aby porównać dwie oferty, potrzebujesz <b>co najmniej dwóch</b> zapisanych wariantów.
                Przejdź do zakładki <b>Kalkulator</b>, wprowadź parametry i kliknij <i>Zapisz kalkulację</i>.
              </div>
        ...
        <div class="cmp-toolbar">
          <div class="cmp-toolbar-group">
            ...
            <div class="seg seg--compact">
              <button class="seg-btn" [class.is-on]="calc.comparison().trendMode === 'overlay'"
                (click)="calc.setComparison({ trendMode: 'overlay' })">nakładka</button>
              <button class="seg-btn" [class.is-on]="calc.comparison().trendMode === 'side-by-side'"
                (click)="calc.setComparison({ trendMode: 'side-by-side' })">obok siebie</button>
            </div>
          </div>

          <label class="cmp-toolbar-toggle">
            <input type="checkbox" [checked]="calc.comparison().showZeroSegments"
          ...
          <label class="cmp-toolbar-toggle">
            <input type="checkbox" [checked]="calc.comparison().diffOnly"
          ...
          <button class="btn btn--ghost btn--mini" title="Drukuj porównanie">
            <svg width="13" height="13" viewBox="0 0 14 14"><path d="M4 5 L4 2 L10 2 L10 5 M4 11 L4 13 L10 13 L10 11 M2 5 L12 5 Q13 5 13 6 L13 9 Q13 10 12 10 L2 10 Q1 10 1 9 L1 6 Q1 5 2 5 Z M4 8 L10 8" stroke="currentColor" stroke-width="1.1" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>
            Drukuj
          </button>
        ...
        @if (calc.offerA()!.source.loanAmount !== calc.offerB()!.source.loanAmount) {
          <div class="cmp-banner cmp-banner--info">
            <svg width="14" height="14" viewBox="0 0 14 14"><circle cx="7" cy="7" r="6" stroke="currentColor" stroke-width="1.3" fill="none"/><path d="M7 4 L7 7.5 M7 9.5 L7 10" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>
            Oferty mają różne kwoty kredytu (<b>{{ fmt0(calc.offerA()!.source.loanAmount) }} zł</b> vs <b>{{ fmt0(calc.offerB()!.source.loanAmount) }} zł</b>) — porównuj ostrożnie.
          </div>
        }
        ...
        <section class="cmp-card cmp-card--params">
          <div class="cmp-card-head">
            ...
            <div class="cmp-diff-pill mono">{{ diffCount() }} z {{ paramRowsAll().length }} różnic</div>
          </div>

          <div class="cmp-params-tbl">
            <div class="cmp-params-head">
              <div>Parametr</div>
              <div class="cmp-col-a"><span class="cmp-col-badge cmp-col-badge--a">A</span> {{ calc.offerA()!.name }}</div>
              <div class="cmp-col-b"><span class="cmp-col-badge cmp-col-badge--b">B</span> {{ calc.offerB()!.name }}</div>
            ...
            @for (g of paramGroups(); track g.group) {
              ...
              @for (r of g.rows; track r.label) {
                <div class="cmp-params-row" [class.is-eq]="r.eq">
                  ...
                  <div class="cmp-params-delta cmp-col-delta mono" [ngClass]="r.deltaClass">{{ r.delta }}</div>
                </div>
              }
            }
          </div>
        ...
        <section class="cmp-card cmp-card--kpi">
          <div class="cmp-card-head">
            <div>
              ...
              <h2 class="cmp-card-title">Cztery wartości, na które patrzą banki — i Twój portfel</h2>
            </div>
            <div class="muted small">Zielony ✓ oznacza lidera w danej metryce (mniej = lepiej).</div>
          </div>
            ...
            @for (m of kpiMetrics(); track m.key) {
              <div class="cmp-kpi-row" [ngClass]="'cmp-kpi-row--' + m.tone">
                <div class="cmp-kpi-card" [class.is-leader]="kpiALeader(m)">
                  ...
                  @if (kpiALeader(m)) { <div class="cmp-kpi-leader">✓ lider</div> }
                  <div class="cmp-kpi-lab"><span class="kpi-dot"></span>{{ m.label }}</div>
                  <div class="cmp-kpi-val mono">{{ fmt0(m.aVal) }}<span class="kpi-unit">zł</span></div>
                ...
                <div class="cmp-kpi-delta" [ngClass]="kpiDeltaClass(m)">
                  ...
                  <div class="cmp-kpi-delta-val mono">{{ kpiDeltaVal(m) }} <span class="muted">zł</span></div>
                ...
                <div class="cmp-kpi-card" [class.is-leader]="kpiBLeader(m)">
                  ...
                  @if (kpiBLeader(m)) { <div class="cmp-kpi-leader">✓ lider</div> }
                  <div class="cmp-kpi-lab"><span class="kpi-dot"></span>{{ m.label }}</div>
                  <div class="cmp-kpi-val mono">{{ fmt0(m.bVal) }}<span class="kpi-unit">zł</span></div>
        ...
        <section class="cmp-card cmp-card--donuts">
          <div class="cmp-card-head">
            <div>
              ...
              <p class="cmp-card-sub">Z czego składa się łączny koszt finansowania w całym okresie.</p>
            </div>
          ...
          <div class="cmp-donut-row">
            <div class="cmp-donut-col cmp-donut-col--a">
              ...
              <app-donut [data]="allDonutA()" [size]="180" [thickness]="26"
                centerLabel="Razem" [centerValue]="donutCenterA()"/>
              <div class="cmp-donut-foot">
                <div class="muted small">Σ płatności</div>
                <div class="mono cmp-donut-total"><b>{{ fmt0(calc.offerA()!.result.totalPayments) }}</b> zł</div>
              </div>
            </div>
            ...
            <div class="cmp-donut-col cmp-donut-col--b">
              ...
              <app-donut [data]="allDonutB()" [size]="180" [thickness]="26"
                centerLabel="Razem" [centerValue]="donutCenterB()"/>
              <div class="cmp-donut-foot">
                <div class="muted small">Σ płatności</div>
                <div class="mono cmp-donut-total"><b>{{ fmt0(calc.offerB()!.result.totalPayments) }}</b> zł</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="cmp-card cmp-card--donuts">
          ...
          <div class="cmp-donut-row">
            <div class="cmp-donut-col cmp-donut-col--a">
              ...
              <app-donut [data]="firstDonutA()" [size]="150" [thickness]="22"
                centerLabel="rata" [centerValue]="fmt0(firstInstallmentA())"/>
              <div class="cmp-donut-foot">
                <div class="muted small">pierwsza rata</div>
                <div class="mono cmp-donut-total"><b>{{ fmt0(firstInstallmentA()) }}</b> zł</div>
              </div>
            </div>
            ...
            <div class="cmp-donut-col cmp-donut-col--b">
              ...
              <app-donut [data]="firstDonutB()" [size]="150" [thickness]="22"
                centerLabel="rata" [centerValue]="fmt0(firstInstallmentB())"/>
              <div class="cmp-donut-foot">
                <div class="muted small">pierwsza rata</div>
                <div class="mono cmp-donut-total"><b>{{ fmt0(firstInstallmentB()) }}</b> zł</div>
              </div>
            </div>
          </div>
        </section>
          ...
          <section class="cmp-card cmp-card--trend">
            <div class="cmp-card-head">
              <div>
                ...
                <p class="cmp-card-sub">Oś X obejmuje unię lat obu ofert ({{ trendOverlay().minYear }}–{{ trendOverlay().maxYear }}). Skala wspólna.</p>
              </div>
              <ul class="cmp-trend-legend">
                <li>
                  <svg width="22" height="6"><line x1="1" y1="3" x2="21" y2="3" stroke="var(--offer-a)" stroke-width="2.4"/></svg>
                  A: {{ calc.offerA()!.name }}
                </li>
                <li>
                  <svg width="22" height="6"><line x1="1" y1="3" x2="21" y2="3" stroke="var(--offer-b)" stroke-width="2.4" stroke-dasharray="3 3"/></svg>
                  B: {{ calc.offerB()!.name }}
                </li>
              </ul>
            </div>
            ...
            @if (trendOverlay(); as o) {
              <svg [attr.viewBox]="'0 0 ' + o.w + ' ' + o.h" class="trend-chart trend-chart--combo" preserveAspectRatio="none">
                @for (t of o.ticks; track t.value) {
                  <line [attr.x1]="o.padL" [attr.x2]="o.w - o.padR"
                        [attr.y1]="t.y" [attr.y2]="t.y"
                        stroke="var(--grid)" stroke-dasharray="2 4"/>
                  <text [attr.x]="o.padL - 10" [attr.y]="t.y + 3" class="ax-label" text-anchor="end">{{ t.label }}</text>
                }

                <text [attr.x]="o.padL - 70" [attr.y]="o.padT + o.innerH / 2"
                      [attr.transform]="'rotate(-90 ' + (o.padL - 70) + ' ' + (o.padT + o.innerH / 2) + ')'"
                      class="ax-title" text-anchor="middle">Kwota pozostała do spłaty</text>

                <!-- B (dashed) -->
                <path [attr.d]="o.pathB" fill="none" stroke="var(--offer-b)" stroke-width="2.4" stroke-dasharray="6 5" stroke-linejoin="round" stroke-linecap="round"/>
                @for (p of o.ptsBVisible; track $index) {
                  <circle [attr.cx]="p.x" [attr.cy]="p.y" r="3.5" fill="var(--surface)" stroke="var(--offer-b)" stroke-width="1.8"/>
                }
                <!-- A (solid) -->
                <path [attr.d]="o.pathA" fill="none" stroke="var(--offer-a)" stroke-width="2.4" stroke-linejoin="round" stroke-linecap="round"/>
                @for (p of o.ptsAVisible; track $index) {
                  <circle [attr.cx]="p.x" [attr.cy]="p.y" r="3.5" fill="var(--offer-a)" stroke="var(--surface)" stroke-width="1.5"/>
                }

                @for (t of o.xTicks; track t.year) {
                  <text [attr.x]="t.x" [attr.y]="o.padT + o.innerH + 10"
                        [attr.transform]="'rotate(-90 ' + t.x + ' ' + (o.padT + o.innerH + 10) + ')'"
                        class="ax-label" text-anchor="end">{{ t.year }}</text>
                }

                <line [attr.x1]="o.padL" [attr.x2]="o.w - o.padR" [attr.y1]="o.padT + o.innerH" [attr.y2]="o.padT + o.innerH" stroke="var(--line-2)"/>
              </svg>
            }
          </section>
        } @else {
          <section class="cmp-card cmp-card--trend">
            <div class="cmp-card-head">
              <div>
                ...
                <p class="cmp-card-sub">Oba wykresy mają wspólną skalę osi, aby porównanie kształtów było uczciwe.</p>
              </div>
            </div>
        ...
        <section class="cmp-card cmp-card--diff">
          ...
          <div class="cmp-diff-tbl">
            <div class="cmp-diff-head">
              <div>Pozycja</div>
              <div class="cmp-col-a"><span class="cmp-col-badge cmp-col-badge--a">A</span> Oferta A</div>
              <div class="cmp-col-b"><span class="cmp-col-badge cmp-col-badge--b">B</span> Oferta B</div>
              <div class="cmp-col-delta">Δ <span class="muted small">(B − A)</span></div>
            </div>
            @for (r of diffRows(); track r.lab) {
              <div class="cmp-diff-row" [class.is-sum]="r.emphasis">
                ...
                <div class="cmp-diff-val cmp-col-a mono" [class.is-leader]="r.aLeader">
                  @if (r.aLeader) { <span class="cmp-diff-marker"></span> }
                  {{ r.aTxt }} {{ r.unit }}
                </div>
                <div class="cmp-diff-val cmp-col-b mono" [class.is-leader]="r.bLeader">
                  @if (r.bLeader) { <span class="cmp-diff-marker"></span> }
                  {{ r.bTxt }} {{ r.unit }}
                </div>
                <div class="cmp-diff-delta cmp-col-delta mono" [ngClass]="r.deltaClass">{{ r.deltaTxt }}</div>
              </div>
            }
          </div>
        </section>
                  ...
                  <button class="cmp-picker-row"
                    ...
                    <div class="cmp-picker-row-main">
                      <div class="cmp-picker-row-name">
                        {{ o.name }}
                        @if (isCurrent(o, slot)) { <span class="cmp-picker-now">obecnie</span> }
                        @if (isBlocked(o, slot)) { <span class="cmp-picker-blocked">już po drugiej stronie</span> }
                      </div>
                      <div class="cmp-picker-row-meta mono muted small">
                        {{ fmt0(o.source.loanAmount) }} zł · {{ periodTxt(o.source.years, o.source.months) }} · {{ fmtPct(o.source.rate, 2) }}%
                      </div>
                    </div>
      ...
      <ng-template #slotTpl let-side="side" let-offer="offer">
        @if (offer) {
          <div class="cmp-slot cmp-slot--filled" [ngClass]="'cmp-slot--' + lower(side)">
            <span class="cmp-slot-badge" [ngClass]="'cmp-slot-badge--' + lower(side)">{{ side }}</span>
            ...
            <div class="cmp-slot-actions">
              <button class="cmp-slot-link" title="Otwórz w kalkulatorze">
                <svg width="11" height="11" viewBox="0 0 12 12"><path d="M2 6 L10 6 M7 3 L10 6 L7 9" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>
                Otwórz
              </button>
              <button class="cmp-slot-clear" (click)="clearSlot($event, side)" title="Wyczyść slot" aria-label="Wyczyść">×</button>
            </div>
          </div>
        } @else {
          <button class="cmp-slot cmp-slot--empty" [ngClass]="'cmp-slot--' + lower(side)" (click)="openPicker(side)">
            <span class="cmp-slot-badge" [ngClass]="'cmp-slot-badge--' + lower(side)">{{ side }}</span>
...
export class ComparisonComponent {
  ...
  openPicker(side: 'A' | 'B') { this.pickerSlot.set(side); }
  ...
  onEsc() { if (this.pickerSlot()) this.pickerSlot.set(null); }

  /* ============ Parametry (3.3) ============ */
  paramRowsAll = computed<ParamRow[]>(() => {
    const a = this.calc.offerA(); const b = this.calc.offerB();
    if (!a || !b) return [];
    const ai = a.source, bi = b.source;
    const out: ParamRow[] = [];

    const numRow = (group: string, label: string, av: number, bv: number,
                    fmtFn: (v: number) => string = fmt0, unit = 'zł', invert = false): ParamRow => {
      const d = bv - av;
      const eq = Math.abs(d) < 0.5;
      const u = unit ? ' ' + unit : '';
      return {
        group, label, kind: 'num',
        a: fmtFn(av) + u, b: fmtFn(bv) + u,
    ...
    const periodRow = (): ParamRow => {
      const am = ai.years * 12 + ai.months, bm = bi.years * 12 + bi.months;
      const d = bm - am;
      const eq = d === 0;
      return {
        group: 'Dane podstawowe', label: 'Okres kredytowania', kind: 'num',
        a: periodTxt(ai.years, ai.months), b: periodTxt(bi.years, bi.months),
    ...
    const dateRow = (label: string, ad: Date, bd: Date): ParamRow => {
      ...
      const eq = dm === 0;
      return {
        group: 'Dane podstawowe', label, kind: 'num',
        a: monthLabel(ad), b: monthLabel(bd),
    ...
    out.push(numRow('Dane podstawowe', 'Wartość nieruchomości', ai.propertyValue, bi.propertyValue));
    out.push(numRow('Dane podstawowe', 'Kwota kredytu', ai.loanAmount, bi.loanAmount));
    out.push(pctRow('Dane podstawowe', 'LTV', ai.loanAmount / ai.propertyValue * 100, bi.loanAmount / bi.propertyValue * 100));
    out.push(periodRow());
    ...
    out.push(txtRow('Nadpłaty', 'Reguła nadpłat (miesięczna)',
  ...
  paramGroups = computed(() => {
    ...
    for (const r of rows) {
      ...
      if (!g) { g = { group: r.group, rows: [] }; groups.push(g); }
      g.rows.push(r);
    }
    return groups;
  });
  ...
  kpiMetrics = computed<KpiMetric[]>(() => {
    const a = this.calc.offerA(); const b = this.calc.offerB();
    if (!a || !b) return [];
    const ai = a.source, bi = b.source, ar = a.result, br = b.result;
    return [
      {
        key: 'first', label: 'Pierwsza rata', tone: 'primary',
        aVal: ar.firstInstallment, bVal: br.firstInstallment,
        aMeta: `${ai.installmentType} · ${ai.rateType} ${fmtPct(ai.rate, 2)}%`,
        bMeta: `${bi.installmentType} · ${bi.rateType} ${fmtPct(bi.rate, 2)}%`,
      },
      {
        key: 'total', label: 'Suma wszystkich płatności', tone: 'cap',
        aVal: ar.totalPayments, bVal: br.totalPayments,
      ...
      {
        key: 'int', label: 'Odsetki', tone: 'int',
        aVal: ar.totalInterest, bVal: br.totalInterest,
      ...
      {
        key: 'costs', label: 'Koszty okołokredytowe', tone: 'cost',
        aVal: ar.totalCosts, bVal: br.totalCosts,
  ...
  allDonutDelta = computed(() => this.donutDeltaRows(this.allSegs(this.calc.offerA()!), this.allSegs(this.calc.offerB()!)));
  firstDonutDelta = computed(() => this.donutDeltaRows(this.firstSegs(this.calc.offerA()!), this.firstSegs(this.calc.offerB()!)));
  private donutDeltaRows(segA: DonutSlice[], segB: DonutSlice[]) {
    return segA.map(sa => {
      const sb = segB.find(s => s.label === sa.label) || { value: 0, color: sa.color, label: sa.label };
  ...
  trendOverlay = computed(() => {
    ...
    const startBalA = (ar.rows[0]?.balance ?? 0) + (ar.rows[0]?.principal ?? 0) + (ar.rows[0]?.overpayment ?? 0);
    const startBalB = (br.rows[0]?.balance ?? 0) + (br.rows[0]?.principal ?? 0) + (br.rows[0]?.overpayment ?? 0);
    ...
    for (let v = 0; v <= yMax + 0.5; v += 50000) {
      ticks.push({ value: v, y: yScale(v), label: new Intl.NumberFormat('pl-PL').format(v) + ' zł' });
    }
    const ptsA = [
    ...
    return {
      ...
      minYear, maxYear,
      ticks, xTicks,
      pathA: path(ptsA), pathB: path(ptsB),
      ptsAVisible: ptsA.slice(1), ptsBVisible: ptsB.slice(1),
    };
  });
  ...
  diffRows = computed<DiffRow[]>(() => {
    ...
    const make = (lab: string, aV: number, bV: number,
                  opts: { invert?: boolean; unit?: string; emphasis?: boolean; fmt?: (v: number) => string } = {}): DiffRow => {
      ...
      return {
        lab, aV, bV, invert, unit, emphasis: opts.emphasis, fmt: fmtFn,
        deltaClass: 'cmp-delta--' + k,
        aLeader, bLeader,
        aTxt: fmtFn(aV), bTxt: fmtFn(bV),
    ...
    return [
      ...
      make('SUMA — całkowity koszt kredytu', ar.totalPayments, br.totalPayments, { emphasis: true }),
      make('Oddasz do banku', oddasA, oddasB, { emphasis: true, unit: '%', fmt: (v) => fmtPct(v, 1) }),
    ];
  });
}
