...
interface ChartGeo {
  balancePath: string; balanceArea: string;
  interestPath: string; principalPath: string;
    ...
    <div class="card">
      <div class="card-head">
        <div>
          ...
          <div class="muted small">{{ calc.startDate() | monthLabel }} → {{ endDate() | monthLabel }}</div>
        </div>
      ...
      @if (geo(); as g) {
        <svg viewBox="0 0 720 240" class="trend-chart" preserveAspectRatio="none">
          ...
          @for (l of g.balLabels; track l.y) {
            <line [attr.x1]="56" [attr.x2]="704" [attr.y1]="l.y" [attr.y2]="l.y" stroke="var(--grid)" stroke-dasharray="2 4"/>
            <text [attr.x]="48" [attr.y]="l.y + 3" class="ax-label" text-anchor="end">{{ (l.v / 1000).toFixed(0) }}k</text>
          }
          <path [attr.d]="g.balanceArea" fill="url(#balGradA)"/>
          <path [attr.d]="g.balancePath" fill="none" stroke="var(--accent-sage-deep)" stroke-width="2"/>
          <path [attr.d]="g.interestPath" fill="none" stroke="var(--accent-peach-deep)" stroke-width="1.5" opacity="0.85"/>
          <path [attr.d]="g.principalPath" fill="none" stroke="var(--accent-lav-deep)" stroke-width="1.5" opacity="0.85"/>
          @for (t of g.yearTicks; track t.year) {
            <text [attr.x]="t.x" [attr.y]="232" text-anchor="middle" class="ax-label">{{ t.year }}</text>
          }
        </svg>
      } @else { <div class="chart-empty">Brak danych</div> }
    </div>
  `,
})
export class TrendChartComponent {
  ...
  geo = computed<ChartGeo | null>(() => {
    ...
    const balancePath = rows.map((r, i) => `${i === 0 ? 'M' : 'L'} ${x(i).toFixed(1)} ${yBal(r.balance).toFixed(1)}`).join(' ');
    const balanceArea = balancePath + ` L ${x(n - 1).toFixed(1)} ${padT + innerH} L ${padL} ${padT + innerH} Z`;
    const interestPath = rows.map((r, i) => `${i === 0 ? 'M' : 'L'} ${x(i).toFixed(1)} ${yR(r.interest).toFixed(1)}`).join(' ');
    const principalPath = rows.map((r, i) => `${i === 0 ? 'M' : 'L'} ${x(i).toFixed(1)} ${yR(r.principal).toFixed(1)}`).join(' ');
